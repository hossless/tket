...
def update_user_profile(request):
    ...
    if request.method == 'GET':
        ...
        if cached_profile:
            return JsonResponse({"user": json.loads(cached_profile)}, status=200)
            
        try:
            with connection.cursor() as cursor:
                cursor.execute("SELECT user_id, username, email, phone_number, first_name, last_name, city FROM users WHERE user_id = %s", [user_id])
