  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pb-20">
      ...
      <div className="py-24 flex flex-col md:flex-row items-center justify-between gap-12 relative">
        ...
        <div className="hidden md:block md:w-1/2 relative h-80 perspective-1000">
      ...
      <div className="mb-16 space-y-5 relative z-20 -mt-10 md:mt-0">
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-4">
          ...
          <button 
            type="submit"
            className="bg-[#8B5CF6] dark:bg-[#B794F4] text-white dark:text-[#1A1924] px-5 py-5 rounded-2xl font-bold hover:bg-[#7C3AED] dark:hover:bg-[#9F7AEA] transition-colors shadow-lg text-lg"
          >
            Search
          </button>
        </form>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
          {['Football', 'Basketball', 'Volleyball'].map(sport => (
            <button 
              ...
              className="px-6 py-2.5 rounded-full font-semibold text-sm transition-all whitespace-nowrap bg-[#FFFFFF] dark:bg-[#232130] text-[#6B7280] dark:text-[#A2A2CC] border border-[#E5E7EB] dark:border-[#2D2B3D] hover:border-[#8B5CF6] dark:hover:border-[#B794F4] hover:text-[#8B5CF6] dark:hover:text-[#B794F4] shadow-sm"
            >
              {sport}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mt-16 text-center">
            <Link 
              to="/search" 
              className="inline-flex items-center gap-2 border border-[#8B5CF6] dark:border-[#B794F4] text-[#8B5CF6] dark:text-[#B794F4] px-8 py-3.5 rounded-full font-bold hover:bg-[#8B5CF6] hover:text-white dark:hover:bg-[#B794F4] dark:hover:text-[#1A1924] transition-all group"
            >
              Browse All Tickets 
